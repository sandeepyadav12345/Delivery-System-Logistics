<?php

return [
    'label'   => 'Local Shops',
    'all'     => 'All Local Shops',
    'single'  => "Local Shop",
    'create'  => "New Local Shop",
    'save'    => "Save Local Shop",
    'delete'  => "Delete Local Shop",
    'deleted' => "<i class='fa-check-circle'></i> Local Shop has been deleted successfully!",

    'dashboard' => 'Local Shop Dashboard',

    'account_number'   => 'Account Number',
    'c_account_number' => 'Local Shop Account Number',
    'trade_name'       => 'Trade Name',
    'national_id'      => 'National ID',
    'password'         => 'Password',
    'name'             => 'Local Shopkeeper Name',
    'phone'            => 'Phone',
    'email'            => 'Email',
    'country'          => 'Country',
    'city'             => 'City',
    'address_sub'      => 'Sub-Address',
    'address_maps'     => 'Address on Google Maps',

    'secondary_emails'      => 'Secondary Emails',
    'secondary_emails_help' => 'Separate emails with comas (e.g: mail@example.com,another@mail.com )...',

    'maps_placeholder' => 'http://maps.google.com/...',

    'zone'                => 'Local Shop Zone',
    'pickup_address_text' => 'Pickup Address',
    'pickup_address_maps' => 'Pickup Address on Google Maps',
    'sector'              => 'Sector',
    'category'            => 'Category',
    'online_store'        => 'Online Store',
    'local_store'         => 'Local Store',

    'bank' => [
        'info'           => 'Bank Information',
        'name'           => 'Bank Name',
        'account_number' => 'Bank Account Number',
        'holder_name'    => 'Account Holder Name',
        'iban'           => 'IBAN',
        'swift_code'     => 'Swift Code'
    ],

    'urls' => [
        'website'   => 'Client Website',
        'facebook'  => 'Facebook Account',
        'instagram' => 'Instagram Account',
    ],

    'files'             => 'Client Attachments',
    'delete_attachment' => "Delete Attachment",

    'charged_for'     => [
        'rejected'         => 'Charged for rejected shipments',
        'cancelled'        => 'Charged for cancelled shipments',
        'returned'         => 'Charged for returned shipments',
        'fixed_value'      => 'Fixed Value',
        'percentage_value' => 'Percentage Value',
        'value'            => 'Value to charge per shipment',
    ],
    "go_to_dashboard" => "Go to :Local Shops Dashboard",
    "address"         => "Local Shops Address",

    // tabs
    'statistics'      => "Statistics",
    'shipments'       => 'Shipments',
    'pickups'         => 'Pickups',
    'zones'           => 'Zones',
    'services'        => 'Services',
    'edit'            => "Edit",
    'properties'      => "Properties",

    'personal'    => "Personal",
    'accounting'  => "Accounting",
    'attachments' => "Attachments",
    'emails'      => "Emails Options",

    'shipment' => "Shipment",

    'statusesPercentage' => 'Number of Shipments',

    'upload'      => 'Upload',
    'upload_file' => 'Add Attachment',

    'client_paid'         => "Mark as Paid",
    'client_paid_tooltip' => "Mark these shipment as Paid for selected client",

    'min_delivery_cost'      => 'Min delivery cost',
    'max_returned_shipments' => 'Max NO. of returned',

    'notes_for_courier' => "Notes for Courier",

    'payment_method'       => "Payment Method",
    'payment_method_price' => "Payment Method Cost",
    'payment_methods'      => [
        'cash'                   => "Cash",
        'bank_deposit'           => "Bank Deposit",
        'bank_transfer'          => "Bank Transfer",
        'exchange_shop_transfer' => "Exchange Shop Transfer",
        'zain_cash'              => "Zain Cash",
        'cash_from_office'       => 'Cash from office'
    ]
];