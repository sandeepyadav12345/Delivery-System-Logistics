<?php

return [
    'label'         => 'Pickups',
    'all'           => 'All Pickups',
    'single'        => "Pickup",
    'today_pickups' => "Today's Pickups",
    'create'        => "New Pickup",
    'edit'          => "Edit Pickup",
    'history'       => "Pickup History",
    'change_log'    => 'Pickup Change Log',
    'delete'        => "Delete Pickup",
    'save'          => "Save Pickup",
    'update'        => "Update Pickup",

    'completed' => 'Completed',
    'pending'   => 'Pending',
    'declined'  => 'Declined',
    'cancelled' => 'Cancelled',

    'pickup_status'        => 'Pickup Status',
    'status_note'          => 'Status Notes',
    'available_time_start' => 'Available Time Start',
    'available_time_end'   => 'Available Time End',

    'statuses' => [
        "created"        => "Pickup Created",
        "collected"      => "Collected",
        "ready"          => "Ready for collection",
        "rescheduled"    => "Rescheduled",
        "pass_to_office" => "Local Shops will pass shipments to Courier",
        "rejected"       => "Rejected",
        "cancelled"      => "Cancelled",
        "not_available"  => "Client is not available",
        "failed"         => "Pickup failed",
        "on_hold"        => "On hold",
        "completed"      => "Completed",
    ],

    'show_more' => "Pickup Details",

    'expected'                  => "Expected",
    'expected_packages_number'  => "Expected Packages Number",
    'actual'                    => "Actual",
    'actual_packages_number'    => "Actual Packages Number",
    'identifier'                => "Identifier",
    'pickup_from'               => "Pick up From",
    'from_client'               => "Local Shopkeeper Himself",
    'from_client_description'   => "Pick up from the client himself.",
    'from_customer'             => "Customer of the Local shop",
    'from_customer_description' => "Pick up from one of Local shop customers.",
    'available_time'            => "Available Time",
    'client_phone'              => "Local Shop Phone",
    'customer_phone'            => "Customer Phone",
    'not_picked_up_yet'         => "Not Picked Up Yet!",
    'client_account_number'     => "Local Shopkeeper Account Number",
    'waybills'                  => "Related Shipments (Waybills)",
    'courier'                   => "Courier to Pickup",
    'pickup_fees'               => "Extra Fees",
    'client_name'               => "Name",
    'phone'                     => "Phone Number",
    'address_text'              => "Address (Text)",
    'address_maps'              => "Address (Google Maps Link)",
    'notes'                     => "Your notes",
    'internal_notes'            => "Notes for courier",
    'external_notes'            => "Courier Notes",
    'address'                   => "Address",
    'prepaid_cash'              => 'Cash on pick up',

    'assignCourierTitle' => "Assign Courier to Pickup"
];